...
async function writeChart(
  ...
  try {
    ...
    let onset: Duration = Duration.fromISO(
      "PT" + duration_chart.onset.toUpperCase()
    );
    let coming_up: Duration = Duration.fromISO(
      "PT" + duration_chart.coming_up.toUpperCase()
    );
    let plateau: Duration = Duration.fromISO(
      "PT" + duration_chart.plateau.toUpperCase()
    );
    ...
    let total_duration_in_seconds: number =
      (max.toMillis() - now.toMillis()) / 1000;
    ...
    const buffer: Buffer = await chartJSNodeCanvas.renderToBuffer(
      configuration
    );
